...
class UserStatus extends Base
{
    ...
    public function getMiddleware()
    {
        return function (Request $request)  {
            if ( $request->getMethod() != Request::METHOD_OPTIONS) {
                ...
                if ($this->redirectToSubscribe()) {
                    return new Response(json_encode([
                        'type' => 'subscribe',
                        'shortDescription' => OW::getLanguage()->text('skmobileapp', 'redirect_to_subscribe'),
                    ]), Response::HTTP_FORBIDDEN, [
                        'Content-Type' => 'application/json'
                    ]);
                }
    ...
    private function redirectToSubscribe()
    {
        //return false;
        $user = $this->getUser();

        if($user){
            return SUBRED_BOL_Service::getInstance()->redirectToSubscribe($user->getId());
        }
